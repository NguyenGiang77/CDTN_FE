...
import './CategorySchedule.scss';
import moment from 'moment';
import {getSchCategoryByDate} from '../../../services/userService'
...
class CategorySchedule extends Component {
    constructor(props) { 
     ...
     async componentDidMount() {
        let { language } = this.props;
         ...
         {
             let res = await getSchCategoryByDate(this.props.inforCategoryCheck, allDays[0].value)
             this.setState({
    ...
    handleOnChangeSelect = async (event) => {
        if (this.props.inforCategoryCheck && this.props.inforCategoryCheck !==-1) {
            let doctorId = this.props.inforCategoryCheck;
            let date = event.target.value;
            let res = await getSchCategoryByDate(doctorId, date);
            ...
            <>
                ...
                <div className='all-time'>
                    <div className='text-celendar'>
                        <i className="fas fa-calendar"><span><FormattedMessage id = "manage-schedule.calendar" /></span></i>
                    </div>
                                ...
                                <div className='time-celendar'>
                                    {allAvalableTime.map((item, index) => {
                                        let timeDisplay = language === LANGUAGES.VI ?
                                            item.timeScheduleCateData.valueVN : item.timeScheduleCateData.valueEN;
                                        return (
                                            <button key={index} 
...
export default connect(mapStateToProps, mapDispatchToProps)(CategorySchedule);
