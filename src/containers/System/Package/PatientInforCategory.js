...
import './PatientInforCategory.scss';
import NumberFormat from 'react-number-format';
import { LANGUAGES } from '../../../utils';
import moment, { lang } from 'moment';
import { sendRemedyCategory, getlisPatientForCategory } from '../../../services/userService';
import { FormattedMessage } from 'react-intl';
import DatePicker from '../../../components/Input/DatePicker';
import RemedyModalInforCategory from './RemedyModalInforCategory';
import { toast } from 'react-toastify';
import LoadingOverlay from 'react-loading-overlay';
import { flatMap } from 'lodash';
class PatientInforCategory extends Component {
    constructor(props) { 
        super(props);
        this.state = {
    ...
    getDataPatient = async () =>{
        ...
        let res = await getlisPatientForCategory({
            inforCategoryId: user.id,
            date:formatedDate
        })
    ...
    handleConfirm = (item) =>{
        let data = {
            inforCategoryId: item.inforCategoryId,
            patientId: item.patientId,
            email: item.patientbookingCategoryData.email,
            timeType: item.timeType,
            patientName: item.patientbookingCategoryData.firstName
        }
        this.setState({
            isOpenModal: true,
            dataModal : data
    ...
    sendRemedyCategory = async (data) =>{
        let {dataModal} = this.state
        this.setState({
            isShowLoading: true
        })
        let res = await sendRemedyCategory ({
            email: data.email,
            imgBase64: data.imgBase64,
            inforCategoryId: dataModal.inforCategoryId,
    ...
    render() {
        ...
        return (
            <>
                                        ...
                                        {dataPatient && dataPatient.length>0 ?
                                            ...
                                            return (
                                                <tr key = {index}>
                                                    <td>{index+1}</td>
                                                    <td>{time}</td>
                                                    <td>{item.patientbookingCategoryData.firstName}</td>
                                                    <td>{gender}</td>
                                                    <td>{item.patientbookingCategoryData.address}</td>
                                                    <td>{item.patientbookingCategoryData.phoneNumber}</td>
                                                    <td>
                                                        <button
                ...
                <RemedyModalInforCategory 
                    isOpenModal={isOpenModal}
                    dataModal={dataModal}
                    closeModal ={this.closeModal}
                    sendRemedyCategory = {this.sendRemedyCategory}
                />                    
                </LoadingOverlay>
            </>
        );
...
export default connect(mapStateToProps, mapDispatchToProps)(PatientInforCategory);
